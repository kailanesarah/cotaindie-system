import { getSheetData, appendSheetData, getSheetDatabyId, updateSheetDatabyId } from '@/lib/googleSheets'
...
export async function GetClientsById(clientId: string) {
  return await getSheetDatabyId(process.env.SHEET_ID!, 'bd-clients!A2:M', clientId)
}
...
export async function updateClientbyId(clientId: string, values: any[][]) {
  await updateSheetDatabyId(process.env.SHEET_ID!, 'bd-clients!A2:M',clientId, values)
