...
export async function signInWithEmail(email: string, password: string) {
    try {
        ...
        return successResponse(data, 200, "auth", "Login realizado com sucesso");
    } catch (err: any) {
...
export async function signUpWithEmail(data: registerInput) {
    try {
        ...
        return successResponse(authData, 201, "auth", "Usuário registrado com sucesso");
    } catch (err: any) {
        throw errorsResponse(
            err.status || 500,
...
export async function requireSession() {
    try {
        ...
        if (error) {
            throw errorsResponse(500, "Erro ao verificar sessão", { message: error.message });
        }

        if (!session) {
            // Retorna erro 401 sem data nula, ou redireciona no frontend
            throw errorsResponse(401, "Usuário não autenticado", {});
        }

        return successResponse(session, 200, "Sessão válida");
    } catch (err: any) {
        throw errorsResponse(
            err.status || 500,
