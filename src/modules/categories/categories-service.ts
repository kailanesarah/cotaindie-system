...
export async function appendCategoryService(data: CategoryInput) {
    try {
        ...
        if (!parsed.success) {
            throw errorsResponse(400, "Dados da categoria inválidos", parsed.error.format());
        }
...
export async function updateCategoryService(category_id: string, data: CategoryInput) {
    try {
        ...
        return successResponse(updatedEntity, 200, "Categoria atualizada com sucesso");
    } catch (err: any) {
        throw errorsResponse(err.status || 500, err.message || "Erro interno", err.details);
    }
}
...
export async function deleteCategoryService(category_id: string) {
    try {
        ...
        return successResponse(deletedEntity, 204, "Categoria deletada com sucesso");
    } catch (err: any) {
        throw errorsResponse(err.status || 500, err.message || "Erro interno", err.details);
    }
}
